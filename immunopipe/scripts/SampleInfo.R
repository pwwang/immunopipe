...
for (i in seq_len(nrow(samples))) {
    type = as.character(samples[i, 'Type'])
    path = samples[i, 'Path']
    if (type != 'scTCR') {
...
save(samples, tcrdir, file=outfile)

# export sample table
write.table(
    old_samples %>% mutate(Path=mapply(
        function(y) y[1],
        strsplit(as.character(unlist(Path)), '/', fixed=T)
    )),
