...
compute.counts.2 <- function (sample, outdir) {
    pheno.1 <- read.table(file.path(outdir, paste0(sample,".pheno1")),header=TRUE)
    pheno.2 <- read.table(file.path(outdir, paste0(sample,".pheno2")),header=TRUE)
    cell.source <- c(rep("BM",nrow(pheno.1)), rep("WBC",nrow(pheno.2)))
...
args <- commandArgs()
# sample, outdir
compute.counts.2(args[length(args)-1], args[length(args)])
