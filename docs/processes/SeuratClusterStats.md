...
- `clustrees_defaults` *(`ns`)*:
    ...
    - `prefix`: *Default: `_auto`*. <br />
        ...
        This will be ignored when `envs.clustrees` is specified.<br />
    - `<more>`:
        Other arguments passed to `clustree::clustree()`.<br />
...
- `stats_defaults` *(`ns`)*:
    ...
    - `frac` *(`choice`)*: *Default: `none`*. <br />
        How to calculate the fraction of cells.<br />
        - `group`:
            calculate the fraction in each group.<br />
            The total fraction of the cells of idents in each group will be 1.<br />
            When `group-by` is not specified, it will be the same as `all`.<br />
        - `ident`:
            calculate the fraction in each ident.<br />
            The total fraction of the cells of groups in each ident will be 1.<br />
            Only works when `group-by` is specified.<br />
        - `cluster`:
            alias of `ident`.<br />
        - `all`:
            calculate the fraction against all cells.<br />
        - `none`:
            do not calculate the fraction, use the number of cells instead.<br />
    ...
    - `devpars` *(`ns`)*:
        ...
        - `width` *(`type=int`)*: *Default: `800`*. <br />
            The width of the plots.<br />
- `stats` *(`type=json`)*: *Default: `{'Number of cells in each cluster': Diot({'pie': True}), 'Number of cells in each cluster by Sample': Diot({'group-by': 'Sample', 'table': True, 'frac': 'group'})}`*. <br />
...
- `dimplots_defaults` *(`ns`)*:
    ...
    - `use` *(`choice`)*: *Default: `scp`*. <br />
        The function from which package to use for the plot.<br />
        - `seurat`:
            Use `Seurat::DimPlot`.<br />
        - `scp`:
            Use `SCP::CellDimPlot`.<br />
        - `scp3d`:
            Use `SCP::CellDimPlot3D`.<br />
        - `Seurat`:
            Same as `seurat`.<br />
        - `SCP`:
            Same as `scp`.<br />
    ...
    - `na_group`:
        ...
        When `use` is `scp`, any non-None values will be translated as `show_na = True` for `SCP::CellDimPlot`.<br />
        You can use `show_na` directly for `SCP::CellDimPlot`. This option is ignored when `use` is `scp3d`.<br />
    - `split-by`:
        The column name in metadata to split the cells into different plots.<br />
        Not supported for `scp3d`.<br />
    - `shape-by`:
        The column name in metadata to use as the shape.<br />
        Ignored if `use` is `scp` or `scp3d`.<br />
    - `subset`:
        An expression to subset the cells, will be passed to `tidyrseurat::filter()`.<br />
    - `devpars` *(`ns`)*:
        The device parameters for the plots.<br />
    ...
    - `theme_use`: *Default: `theme_blank`*. <br />
        The theme to use for the plot.<br />
    - `<more>`:
        See <https://satijalab.org/seurat/reference/dimplot>
- `dimplots` *(`type=json`)*: *Default: `{'Dimensional reduction plot': Diot({'label': True, 'label_repel': True, 'label-box': True, 'repel': True}), 'TCR presence': Diot({'ident': 'TCR_Presence', 'order': 'TCR_absent', 'cols': ['#FF000066', 'gray']})}`*. <br />
...
![nCells_All](../latest/processes/images/SeuratClusterStats_nCells_All.png){: width="80%" }
...
![nCells_Sample](../latest/processes/images/SeuratClusterStats_nCells_Sample.png){: width="80%" }
...
![vlnplots](../latest/processes/images/SeuratClusterStats_vlnplots.png){: width="80%" }
![vlnplots_1](../latest/processes/images/SeuratClusterStats_vlnplots_1.png){: width="80%" }
...
![dimplots](../latest/processes/images/SeuratClusterStats_dimplots.png){: width="80%" }
