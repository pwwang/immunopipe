...
- `stats_defaults` *(`ns`)*:
    ...
    - `<more>`:
        ...
        See <https://pwwang.github.io/scplotter/reference/CellStatPlot.html>.<br />
- `stats` *(`type=json`)*: *Default: `{'Number of cells in each cluster (Bar Chart)': Diot({'plot_type': 'bar'}), 'Number of cells in each cluster by Sample (Bar Chart)': Diot({'plot_type': 'bar', 'group_by': 'Sample'})}`*. <br />
...
- `dimplots` *(`type=json`)*: *Default: `{'Dimensional reduction plot': Diot({'label': True, 'label_insitu': True, 'label-box': True, 'repel': True}), 'TCR presence': Diot({'ident': 'TCR_Presence', 'order': 'TCR_absent', 'cols': ['#FF000066', 'gray']})}`*. <br />
